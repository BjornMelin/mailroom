...
fn bootstrap_workspace() -> (TempDir, ConfigReport) {
    let temp_dir = TempDir::new().unwrap();
    let paths = WorkspacePaths::from_repo_root(temp_dir.path().to_path_buf());
    paths.ensure_runtime_dirs().unwrap();
    let config_report = resolve(&paths).unwrap();
    (temp_dir, config_report)
}

#[tokio::test]
async fn search_rejects_whitespace_only_terms() {
    let (_temp_dir, config_report) = tokio::task::spawn_blocking(bootstrap_workspace)
        .await
        .unwrap();
    let config_report_for_init = config_report.clone();
    tokio::task::spawn_blocking(move || {
        store::init(&config_report_for_init).unwrap();
    })
    .await
    .unwrap();
...
async fn search_before_date_excludes_that_day() {
    let (_temp_dir, config_report) = tokio::task::spawn_blocking(bootstrap_workspace)
        .await
        .unwrap();
    let config_report_for_seed = config_report.clone();
    tokio::task::spawn_blocking(move || {
        store::init(&config_report_for_seed).unwrap();
        accounts::upsert_active(
            &config_report_for_seed.config.store.database_path,
            config_report_for_seed.config.store.busy_timeout_ms,
...
async fn search_migrates_schema_v2_store_before_querying_mailbox_tables() {
    ...
    let store_report = tokio::task::spawn_blocking({
        let config_report = config_report.clone();
        move || store::inspect(config_report).unwrap()
    })
    .await
    .unwrap();
    assert_eq!(
...
async fn search_uses_local_mailbox_cache_after_logout() {
    ...
    let mock_server_uri = mock_server.uri().to_owned();
    let (_temp_dir, config_report) = tokio::task::spawn_blocking(move || {
        let temp_dir = TempDir::new().unwrap();
        let config_report = config_report_for(&temp_dir, &mock_server_uri);
        (temp_dir, config_report)
    })
    .await
    .unwrap();
    let config_report_for_seed = config_report.clone();
    tokio::task::spawn_blocking(move || {
        seed_credentials(&config_report_for_seed);
        seed_existing_mailbox(
            &config_report_for_seed,
            "250",
            "cached-1",
            "Existing cached message",
        );

        let logout_report = auth::logout(&config_report_for_seed).unwrap();
